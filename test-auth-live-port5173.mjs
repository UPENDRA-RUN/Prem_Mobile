...
async function run() {
  console.log(`\n======================================================`);
  console.log(`VERIFYING ALL 12 ACCEPTANCE TESTS ON VITE SERVER (PORT 5173)`);
  console.log(`======================================================\n`);
  ...
  const testEmail = `testuser_vite_${Date.now()}@example.com`;
  ...
  if (loginMobileRes.ok && loginMobileRes.data.user.name === 'Priya Sharma') {
  ...
  const adminProductRes = await request('/products', {
    ...
    body: JSON.stringify({
      name: `Prem Bass Wireless Earbuds ${Date.now()}`,
      category: 'Earbuds',
      brand: 'Prem Mobile',
      description: 'Ultra bass earbuds with ANC',
      regularPrice: 1499,
      stock: 40,
      isActive: 1,
      isFeatured: 1
  ...
  const catRes = await request('/categories', {
    ...
    headers: { Authorization: `Bearer ${adminToken}` },
    body: JSON.stringify({
      name: `Type-C Cables ${Date.now()}`,
      icon: '🔌'
    })
  });
  if (catRes.status === 201 && catRes.data.category.name.startsWith('Type-C Cables')) {
  ...
  console.log(`\n======================================================`);
  console.log(`RESULTS ON PORT 5173: ${passed} / ${total} TESTS PASSED`);
  console.log(`======================================================\n`);
}
