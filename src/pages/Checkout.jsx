...
export default function Checkout() {
  ...
  const { cartItems, subtotal, finalTotal: cartFinalTotal, clearCart } = useCart();
  ...
  const handleApplyCouponCode = async (codeToApply) => {
    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        setCouponCodeInput(data.data.code);
        setCouponError(null);
      } else {
        setCouponError(data.message || 'Invalid promo coupon');
                ...
                {appliedCoupon ? (
                  <div className="p-3 rounded-2xl bg-emerald-50 border border-emerald-200 flex items-center justify-between">
                    <div className="space-y-0.5">
                      <div className="flex items-center gap-2">
                        <span className="font-mono font-black text-emerald-900 text-xs">{appliedCoupon.code}</span>
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-full hover:bg-emerald-200/60 text-emerald-800 transition"
                      title="Remove coupon"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  ...
                  <div className="space-y-2">
                    <div className="flex gap-2">
                      <input
                        type="text"
                        placeholder="Enter promo code (e.g. GWALIOR10)"
                        value={couponCodeInput}
                        onChange={(e) => setCouponCodeInput(e.target.value.toUpperCase())}
                        className="flex-1 px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs font-mono font-bold uppercase focus:outline-none focus:ring-2 focus:ring-[#ffd000]"
                      />
                      <button
                        type="button"
                        disabled={isValidatingCoupon || !couponCodeInput.trim()}
                        onClick={() => handleApplyCouponCode()}
                        className="px-4 py-2.5 rounded-xl bg-[#050505] hover:bg-slate-800 disabled:bg-slate-200 disabled:text-slate-400 text-[#ffd000] font-black text-xs uppercase tracking-wider transition shadow-sm"
                      >
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 pt-1">
                      ...
                      {['GWALIOR10', 'SUNDAY500', 'WELCOME100'].map((suggestCode) => (
                        <button
                          type="button"
                          key={suggestCode}
                          onClick={() => handleApplyCouponCode(suggestCode)}
                          className="px-2 py-0.5 rounded-md bg-amber-50 hover:bg-amber-100 border border-amber-200 text-amber-900 font-mono font-bold text-[10px] transition"
                        >
                          {suggestCode}
                        </button>
                      ))}
